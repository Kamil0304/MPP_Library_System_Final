...
public class FormAddMemberController extends SaveFormBaseController {
    ...
    public void updateMember(ActionEvent event) {
        ...
        if ( !validate() ) // if not valid don't continu, just return to the form
            return; 
        String a = getId().getText();
        System.out.println(a);
        LibraryMember libraryMember =new MemberDAO().getLibraryMember(getId().getText());
        libraryMember.setFirstName(firstName.getText());
        System.out.println(libraryMember.getFirstName());
        libraryMember.setLastName(lastName.getText());
        libraryMember.setPhoneNumber(phoneNumber.getText());
        libraryMember.setAddress(new Address(street.getText(),city.getValue().toString(),state.getValue().toString(),zip.getText()));
        
        try {
            memberDAO.editMember(libraryMember);
            Message.showSuccessMessage("Update Member", "Updating Member Sucess", "");      
            
        } catch (IOException ex) {
            Message.showErrorMessage("Update Member", "Updating Member Failed. Exception message: ",  ex.getMessage());          
